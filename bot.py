...
async def create_role(interaction: discord.Interaction, rolename: str):
    ...
    print(f"User {interaction.user.display_name} has roles: {user_roles}")
    
    # Check if the invoking user has the "huh" role (case-insensitive)
    has_huh_role = any(role.name.lower() == "huh" for role in interaction.user.roles)
    print(f"User has 'huh' role: {has_huh_role}")
...
async def on_ready():
    ...
    await bot.load_extension("rpgcombat")
